...
class WalletTest(WagerrTestFramework):
    ...
    def run_test(self):
        ...
        assert_equal(len(self.nodes[0].listunspent()), 21)
        ...
        assert_equal(self.nodes[0].getbalance("*"), 180860471)
        assert_equal(self.nodes[0].getbalance("*", 1), 180860471)
        assert_equal(self.nodes[0].getbalance("*", 1, True), 180860471)
        assert_equal(self.nodes[0].getbalance("*", 1, True, False), 180860471)
        ...
        def test_balances(*, fee_node_1=0):
            ...
            # Same with minconf=0
            assert_equal(self.nodes[0].getbalance(minconf=0), Decimal('180859980.99000000'))
            assert_equal(self.nodes[1].getbalance(minconf=0), Decimal('22998039.99997750') - fee_node_1)
        ...
        # self.nodes[1].sendrawtransaction(txs[1]['hex']) # disabled, no RBF in Wagerr
        #self.nodes[0].sendrawtransaction(txs[1]['hex'])  # sending on both nodes is faster than waiting for propagation # disabled, no RBF in Wagerr
        self.sync_all()
        ...
        assert_equal(self.nodes[0].getbalance(minconf=0), Decimal("180860970.86991000"))
