    ...
    def _initialize_chain(self):
        ...
        if not os.path.isdir(cache_node_dir):
            ...
            for i in range(8):
                ...
                self.nodes[CACHE_NODE_ID].generate(25 if i != 7 else 24)
            ...
            for entry in os.listdir(cache_path()):
                if entry not in ['wallets', 'chainstate', 'blocks', 'evodb', 'llmq', 'backups', 'betting', 'tokens', 'indexes', 'zerocoin']:
...
class WagerrTestFramework(WagerrTestFramework):
            ...
            for mn in mninfos:
                ...
                for qs in s:
                    ...
                    if qstatus["quorumHash"] != quorum_hash:
                        continue
                    member_count += 1
                    self.log.info("qstatus %s: " % (qstatus))
                    if "phase" not in qstatus:
                        mn_ok = False
                        break
                    self.log.info("qstatus phase %s: phase %s" % (qstatus["phase"], phase))
    ...
    def mine_quorum(self, llmq_type_name="llmq_test", llmq_type=100, expected_connections=None, expected_members=None, expected_contributions=None, expected_complaints=0, expected_justifications=0, expected_commitments=None, mninfos_online=None, mninfos_valid=None):
        self.nodes[0].sporkupdate('SPORK_21_QUORUM_ALL_CONNECTED', 21)
        self.nodes[0].sporkupdate('SPORK_23_QUORUM_POSE', 21)
