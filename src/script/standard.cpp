...
static bool MatchPayToGrpPubkeyHash(const CScript& script, valtype& pubkeyhash, valtype& group, valtype& groupQty)
{
    ...
    if (!script.GetOp(pc, opcodeGrp, groupId)) return false;
    if (!script.GetOp(pc, opcodeQty, tokenQty)) return false;
    ...
    if (!script.GetOp(pc, opcode, pkh)) return false;
    if (pkh.size() != sizeof(uint160)) return false;
    if (!script.GetOp(pc, opcode, data)) return false;
...
txnouttype Solver(const CScript& scriptPubKey, std::vector<std::vector<unsigned char>>& vSolutionsRet)
{
    ...
    if (MatchPayToGrpPubkeyHash(scriptPubKey, data, group, groupQty)) {
        // group will always be the last entity in vSolutionsRet
        // group quantity will be the second to last
        if (!group.empty())
        {
            vSolutionsRet.push_back(std::move(data));
            vSolutionsRet.push_back(std::move(groupQty));
            vSolutionsRet.push_back(std::move(group));
        }
        return TX_GRP_PUBKEYHASH;
    }
